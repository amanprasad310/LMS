//
//  SuperAdminDashboard.swift
//  LMS
//
//  Created by Rounak Azad on 15/02/25.
//
import SwiftUI

struct SuperAdminDashboardView: View {
    // State to track the selected tab
    @State private var selectedTab: Tab = .dashboard

    // Enum to represent the tabs
    enum Tab {
        case dashboard
        case profile
    }

    var body: some View {
        NavigationView { // Single NavigationView at the top level
            VStack(spacing: 0) {
                // Show the appropriate view based on the selected tab
                switch selectedTab {
                case .dashboard:
                    DashboardView()
                case .profile:
                    SuperAdminProfileView(admin: Admin(name: "Aman", email: "xyz@gmail.com"))
                        .toolbar {
                            // Center the navigation title
                            ToolbarItem(placement: .principal) {
                                Text("Profile")
                                    .font(.title)
                                    .foregroundColor(.brown)
                                    .fontWeight(.bold)
                            }
                        }
                }

                // Bottom Navigation Bar
                bottomNavBar
            }
            .background(Color(UIColor.systemGray6))
            .navigationBarBackButtonHidden(true)
        }
    }
}

// MARK: - Dashboard View
struct DashboardView: View {
    var body: some View {
        VStack {
            // Header
            headerView

            // List of Super Admin Actions
            List {
                adminNavigationItem(
                    title: "Manage your admin",
                    subtitle: "Easily oversee and manage your librarian",
                    icon: "person.2.fill" // Unique icon for "Manage your admin"
                )

                adminNavigationItem(
                    title: "Manage your library",
                    subtitle: "Create and maintain your library",
                    icon: "books.vertical.fill" // Unique icon for "Manage your library"
                )
            }
            .listStyle(InsetGroupedListStyle())
        }
    }
}

// MARK: - UI Components
extension DashboardView {
    private var headerView: some View {
        VStack(alignment: .leading) {
            Text("Welcome Super Admin")
                .font(.title)
                .bold()
                .foregroundColor(.brown)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding()
        .background(Color.white)
    }

    private func adminNavigationItem(title: String, subtitle: String, icon: String) -> some View {
        NavigationLink(destination: destinationView(for: title)) {
            HStack(alignment: .center, spacing: 12) { // Adjusted spacing for better alignment
                Image(systemName: icon)
                    .resizable()
                    .frame(width: 30, height: 30)
                    .foregroundColor(.brown) // Custom icon color
                    .padding(.trailing, 13)

                VStack(alignment: .leading, spacing: 4) { // Adjusted spacing for better alignment
                    Text(title)
                        .font(.headline)
                        .foregroundColor(.black)

                    Text(subtitle)
                        .font(.subheadline)
                        .foregroundColor(.gray)
                }
                .frame(maxWidth: .infinity, alignment: .leading) // Ensure text is aligned to the left
            }
            .padding(.vertical, 10) // Add vertical padding for better spacing
            .padding(.horizontal, 8) // Add horizontal padding for better spacing
        }
    }

    private func destinationView(for title: String) -> some View {
        switch title {
        case "Manage your admin":
            return AnyView(AdminListView()) // Replace with your actual view
        case "Manage your library":
            return AnyView(LibraryListView()) // Replace with your actual view
        default:
            return AnyView(EmptyView())
        }
    }
}

// MARK: - Bottom Navigation Bar
extension SuperAdminDashboardView {
    private var bottomNavBar: some View {
        HStack {
            // Dashboard Tab
            Button(action: {
                selectedTab = .dashboard
            }) {
                bottomNavItem(icon: "house.fill", label: "Dashboard", isActive: selectedTab == .dashboard)
            }
            .frame(maxWidth: .infinity) // Distribute space evenly

            // Profile Tab
            Button(action: {
                selectedTab = .profile
            }) {
                bottomNavItem(icon: "person.crop.circle", label: "Profile", isActive: selectedTab == .profile)
            }
            .frame(maxWidth: .infinity)
        }
        .padding(.vertical, 10) // Add vertical padding
        .background(Color.white)
    }

    private func bottomNavItem(icon: String, label: String, isActive: Bool) -> some View {
        VStack(spacing: 4) { // Adjust spacing between icon and text
            Image(systemName: icon)
                .resizable()
                .frame(width: 24, height: 24)
            Text(label)
                .font(.caption)
        }
        .foregroundColor(isActive ? .brown : .gray)
        .padding(.vertical, 8) // Add padding to the entire tab item
    }
}

// MARK: - Preview
struct SuperAdminDashboardView_Previews: PreviewProvider {
    static var previews: some View {
        SuperAdminDashboardView()
    }
}
